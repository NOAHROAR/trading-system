...
#
# '_degraded' marker (2026-08-26 fix — real data-loss incident, root-caused):
# a ~16h Postgres outage (8/21-8/22) silently destroyed real open positions.
# _load_positions() fell back to a LOCAL JSON file that's only ever updated
# when a *write* fails — frozen at container-boot state (empty) the whole time
# DB writes had been succeeding. _save_positions() is called unconditionally
# from _send_morning_vitals / _check_daily_summary / _attempt_entry regardless
# of whether the in-memory state was actually complete. The moment Postgres
# recovered and one of those unconditional saves fired, it overwrote real
...
# fallback because a DB read that should have succeeded didn't" — and
# _save_positions() refuses to persist anything (DB or JSON) while that flag
# is set, alerting instead of silently destroying data.
...
def _reconcile_on_startup():
    ...
    try:
        ...
        other_legs  = _other_strategy_leg_symbols()   # iron_condor_strat.py's own legs
        option_legs = [
            p for p in r.json()
